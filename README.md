...
- **iOS (iPhone / iPad) では HTTPS 接続を推奨**します。HTTP (例: `http://192.168.1.5:8000`) は secure context ではないため、Service Worker が登録できず **PWA としてのインストール・オフライン動作・wasm-vips が使えません** (ライブラリの閲覧とファイルを開く操作自体は HTTP でも動作します)。HTTPS に対応していない PHP のビルトインサーバー (`php -S`) を使う場合は、cloudflare tunnel 等を併用してください / **HTTPS is recommended on iOS (iPhone / iPad)**: plain HTTP is not a secure context, so the Service Worker cannot register — no PWA install, no offline support, no wasm-vips (browsing the library and opening files still work over HTTP). If you use PHP's built-in server (`php -S`), which has no HTTPS support, put something like a Cloudflare Tunnel in front of it
